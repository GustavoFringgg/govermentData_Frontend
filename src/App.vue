...
// 取得 cache 資料
async function handelCached() {
  loadingCached.value = true;
  error.value = "";
  try {
    const { data, last_updated: cachedLastUpdated } = await fetchCached();
    tenders.value = data;
    hasFetched.value = true;
    last_updated.value = cachedLastUpdated;
  } catch (e: unknown) {
  ...
  <div class="min-h-screen bg-gray-50 text-gray-800">
    ...
    <main class="max-w-7xl mx-auto px-4 py-8">
      ...
      <div
        ...
        data-testid="loading-cached"
      >
        <p class="text-gray-500 text-lg">取得快取資料中，請稍候...</p>
      </div>

      <!-- Loading with progress bar -->
      <div v-if="loading" class="max-w-lg mx-auto py-12" data-testid="loading">
        <div
          class="bg-white rounded-xl shadow p-8 flex flex-col items-center gap-5"
        >
      ...
      <div
        v-if="error"
        class="bg-red-50 border border-red-300 text-red-700 rounded-lg p-4 mb-8 text-center"
        data-testid="error"
      >
        {{ error }}
      </div>
      ...
      <!-- Charts -->
      <div
        v-if="hasFetched && !loading && tenders.length > 0"
        data-testid="charts"
        class="space-y-12"
      >
        ...
        <div class="bg-white rounded-xl shadow overflow-hidden">
            ...
            <table class="w-full text-sm text-left table-fixed min-w-[960px]">
              <thead class="bg-gray-100 text-gray-600">
                <tr>
                  ...
                  >
                    招標方式
                    <span class="text-xs">{{ sortIcon("tender_mode") }}</span>
                  </th>
                  <th
                  ...
                  >
                    預算金額
                    <span class="text-xs">{{ sortIcon("budget") }}</span>
                  </th>
                </tr>
              </thead>
              <tbody>
                ...
                >
                  ...
                  <td class="px-4 py-3 truncate" :title="t.tender_name">
                    {{ t.tender_name }}
                  </td>
          ...
          <!-- Pagination -->
          <div
            class="flex items-center justify-between px-6 py-4 border-t text-sm text-gray-500"
          >
            <span>
              第 {{ (currentPage - 1) * pageSize + 1 }}–{{
                Math.min(currentPage * pageSize, sortedTenders.length)
              }}
              筆，共 {{ sortedTenders.length }} 筆
            </span>
      ...
      <!-- Empty State -->
      <div
        v-if="hasFetched && !loading && tenders.length === 0 && !error"
